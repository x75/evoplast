...
class GenetPlast(Genet):
    def __init__(self, modelsize = 2, state_dim = 2, M = None):
        ...
        self.networks["fast"]["M"] = np.random.uniform(-1e-0, 1e-0, (self.state_dim, self.state_dim)) * 1.0 # 0.01
        ...
        if M is None:
            self.networks["slow"]["M"] = np.random.uniform(-1, 1, (self.networks["slow"]["s_dim"], self.networks["slow"]["i_dim"])) * 0.01 # 1.0 # 
        else:
